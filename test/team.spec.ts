...
describe("Ways to create Team", () => {
    ...
    it("Name w/ Players", () => {
        ...
        let result = new Team("Team 2", player1);
        ...
        result = new (Function.prototype.bind.apply(Team, [null, "Team 1", player1]));

        expect(result.name).equals("Team 1");
        expect(result.members).length(1);
        expect(result.members[0]).contains({name: "Player 1"});

        result = new (Function.prototype.bind.call(Team, null, "Team 1", player2));
...
describe("Team Functions", () => {
    ...
    it("Adds 1 member by Object", () => {
        let playerX = new Player(50, "Player X");
        let uid = uuidv1();
        playerX.setKey(uid);
